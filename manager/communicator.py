...
class ClientConnection:
    ...
    def send_new_input(self, data, bitmap, info, label=None):
        self.sock.send_bytes(msgpack.packb(
            {"type": MSG_NEW_INPUT, "input": {"payload": data, "bitmap": bitmap, "info": info, "label": label}}))  # label = "import"
    ...
    def send_state_sequence_for_update_fuzzs(self, state_sequence):
        """
        Send the state sequence to the manager for updating fuzzs.
        """
        self.sock.send_bytes(msgpack.packb(
            {"type": "UPDATE_FUZZS", "state_sequence": state_sequence}))