...
class ManagerStatistics:
    ...
    def event_choose_state_seed(self): # @@
        self.data["choose_state_seed"] += 1
    ...
    def update_yield(self, node):
        method = node.node_struct["info"]["method"] # TO DO: add node.get_method() API
        if method not in self.data["yield"]:
            self.data["yield"][method] = 0
        self.data["yield"][method] += 1

    def maybe_write_stats(self, state_machine=None):
        cur_time = time.time()
    ...
    def write_plot_header(self): # @@
        with open(self.plot_file, 'a') as fd:
            fd.write("#secs; exec/s; paths; p_pend; favs; crash; kasan; tmout; lvls; cycles; choose_state_seed; f_pend; exec; edges; ipsm_nodes; ipsm_edges\n")

    def write_plot(self, state_machine=None):
        node_count, edge_count = (0, 0) # @@
        if state_machine:
            node_count, edge_count = state_machine.get_node_edge_count()
            self.data["state_nodes"] = node_count  # @@ 디버깅 필요
            self.data["state_edges"] = edge_count  # @@
        cur_time = time.time()
        ...
        with open(self.plot_file, 'a') as fd:
            fd.write("%06d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d\n" % (
                ...
                self.data["choose_state_seed"],# choose_state_seed # @@ 디버깅 필요
                ...
                node_count,                    # state machine nodes # @@
                edge_count,                    # state machine edges # @@
                ))
