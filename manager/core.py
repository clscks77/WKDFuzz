...
def start(settings: LazySettings):  # @@ kafl fuzz -w work --seed-dir seed --redqueen -p nproc --t-soft 255 --purge -i result.json --use_call_stack --t-hard 255
    ...
    print(workdir)  # -w work
    seed_dir   = settings.seed_dir
    print(seed_dir) # --seed-dir seed
    dependency_dir = workdir+"/dependency"
    num_worker = settings.processes # -p nproc
    interface = settings.interface 
    call_stack_mode = settings.use_call_stack # --use_call_stack
    ...
    manager = ManagerTask(settings)     # Manager instance 생성
