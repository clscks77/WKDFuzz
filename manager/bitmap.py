...
class GlobalBitmap:
    ...
    def get_new_byte_and_bit_offsets(self, local_bitmap):
        # TO DO ensure that local_bitmap doesn't need a copy to increase performance
    ...
    def update_with(self, exec_result):
        assert (not self.read_only)
        GlobalBitmap.bitmap_native_so.update_global_bitmap(self.c_bitmap, exec_result.cbuffer, # exec_result.cbuffer == new_bitmap
                                                           ctypes.c_uint64(self.bitmap_size))
...
class BitmapStorage:
    ...
    def check_storage_logic(self, exec_result, new_bytes, new_bits):
        """
        실행 결과를 manager에게 전송할지 여부를 결정하는 로직
        - exit reason이 regular이고, 새로운 byte나 bit가 있다면 전송 (=True)
        - exit reason이 regular이 아니더라도, 새로운 byte가 있다면 전송 (=True)
        """
        if exec_result.exit_reason == "regular" and (new_bits or new_bytes): # regular이고, 새로운 커버리지가 있다.
            return True
        elif new_bytes: # regular는 아닌데, 새로운 branch가 있다.
            return True
        return False    # regular이지만 새로운 byte나 bit가 없을 때, 또는 regular가 아니고 새로운 branch도 없을 때

    def should_send_to_manager(self, exec_result, exit_reason):
        """
        exec_result의 exit_reason에 따라 적절한 비트맵을 선택하고,
        새로운 byte나 bit가 있는지 확인하여, 전송 여부를 결정
        """
        ...
        return self.check_storage_logic(exec_result, new_bytes, new_bits), new_bytes
