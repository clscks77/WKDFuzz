...
class Scheduler:
    ...
    def score_priority_favs(self, node, config):
        ...
        if node.get_exit_reason() != "regular":
            ...
            print("[-] unknown node.get_state(),,,,,,, {node.get_state()}")
        ...
        original_priority = prio*phase/time_buff
        if not config.directed:
            return original_priority
        distance_score = node.get_dg_score()
        final_priority = original_priority / (1 + distance_score)
        if distance_score <= 7 and final_priority < 10:
            final_priority = final_priority + 20
        if distance_score == 0 and final_priority < 30:
            final_priority = final_priority + 80

        node.set_final_priority(final_priority, write=True)
