...
from kafl_fuzzer.common.util import prepare_working_dir, read_binary_file, qemu_sweep, p32
...
def execute_once(config, qemu_verbose=False, notifiers=True):
    ...
    #payload = read_binary_file(payload_file)
    IoControlCode = 0x222818
    InBuffer_length =  8 
    OutBuffer_length = 8
    Inbuffer = b"A"*8
    payload = p32(IoControlCode) + p32(InBuffer_length) + p32(OutBuffer_length) + Inbuffer
...
def start(settings: LazySettings):
    ...
    try:
        # TO DO: noise, benchmark, trace are working, others untested
