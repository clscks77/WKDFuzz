...
// for aux2 buffer
#define HYPERCALL_KAFL_USER_AUX2_PUBLISH  41
...
#pragma pack(push, 1)
typedef struct {
    uint32_t seq_ptr;    /* net_state_sequence 바이트 배열의 GVA */
    uint32_t seq_len;    /* 길이 */
    // uint32_t _rsvd0;
    uint32_t flags_ptr;  /* uint32_t flags[flag_cnt] 의 GVA */
    uint32_t flag_cnt;   /* 개수 */
	uint64_t T_IMG_base;  /* target driver image base */
	uint32_t T_IMG_size;  /* target driver image size */
