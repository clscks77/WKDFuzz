...
class RedqueenInfoGatherer:
    def __init__(self, allowed_ranges=None):
    ...
    def run_mutate_redqueen(self, headers, payload_array, func, worker_pid=None):
        for (offset, lhs, rhs, info) in self.enumerate_mutations():
            #logger.debug("redqueen fuzz data %s" % repr((offset, lhs, rhs, info)))

            # print(f"[** DEBUG] ############################## Redqueen mutation at offsets {offset}") # @@ 디버깅용 
            # print(f"**Worker {worker_pid}, offsets: {offset}", end=' ') # @@ 디버깅용 

            def run(headers, data):
                extra_info = {"redqueen": [repr(lhs), repr(rhs)] + list(info.infos)}
                # print(f"**Worker {worker_pid}, offsets: {offset}", end=' ') # @@ 디버깅용 
                func(headers, data, None, extra_info)  # FuzzingStateLogic.execute_sangjun
                # print(f"** ..Done..**Worker {worker_pid}, offsets: {offset}", end=' ') # @@ 디버깅용 
