...
class RedqueenRunInfo:
    def __init__(self, id, was_colored, hook_info, input_data):
        ...
        self.allowed_ranges = None

    def set_allowed_ranges(self, allowed_ranges):
    ...
    def calc_offsets(self, pattern):
        ...
        if isinstance(pattern, str):
            # TO DO: Find source of str type inputs to avoid this check + conversion
        ...
        while True:
            ...
            if self.allowed_ranges:
                r_start, r_end = self.allowed_ranges
                
                # 'start' 오프셋이 허용된 범위 내에 있는지 확인
                if not (start >= r_start and start < r_end):
                    start += 1  # 범위 밖이면 무시하고 다음 검색
                    continue
            # ===============================================
            res.add(start)
...
class RedqueenInfo:
    ...
    def load_data(self, input_id, was_colored, hook_info, bin_info):
        ...
        run_info.set_allowed_ranges(self.allowed_ranges)
...
def parse_rq(path, num_files, orig_file_id, allowed_ranges=None):
    rq_info = RedqueenInfo()
    rq_info.set_allowed_ranges(allowed_ranges)
