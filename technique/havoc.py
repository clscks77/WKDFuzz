...
def mutate_seq_havoc_array(irp_list, index, func, max_iterations, mutation_part="all", resize=False):
    # if resize:
    #     data = data + data
    # else:
    #     data = data
    try:      # --- IndexError 발생 시 index = 0으로 설정 ---
        if mutation_part == "header":
            data = irp_list[index].header
            ...
            data = irp_list[index].header + irp_list[index].InBuffer
            InBufferLength = irp_list[index].header_length + irp_list[index].InBuffer_length
    except IndexError:
        print(f"[-] Warning: Index {index} was out of bounds. Defaulting to index 0.")
        index = 0
        if mutation_part == "header":
            data = irp_list[index].header
            InBufferLength = irp_list[index].header_length
    ...
    # mutation을 얼마나 중첩시킬지
    stacking = rand.int(AFL_HAVOC_STACK_POW2)   # 5
    stacking = 1 << (stacking)  # # 2^n 형태의 mutation depth 결정 (2, 4, 8, 16, 32)

    for _ in range(1+max_iterations//stacking):
        for _ in range(stacking):
            handler = rand.select(havoc_handler) # 무작위 mutation 함수 선택
            data = handler(data)                 # havoc mutation 적용
            
            if len(data) >= InBufferLength:
                data = data[:InBufferLength]
            else:
                data = data.ljust(InBufferLength,b"\xff") # 부족한 부분을 ff로 채움
            
            if mutation_part == "header":
                irp_list[index].header = data
            elif mutation_part == "body":
                irp_list[index].InBuffer = data
            elif mutation_part == "all":
                orig_header_len = irp_list[index].header_length
                irp_list[index].header = data[:orig_header_len]
                irp_list[index].InBuffer = data[orig_header_len:]

            func(irp_list)                   # FuzzingStateLogic.execute

def mutate_seq_splice_array(irp_list, index, func, max_iterations, mutation_part="all", resize=False):
    ...
    for _ in range(splice_rounds):
        if mutation_part == "header":
            spliced_data = havoc_splicing(header, files)
        elif mutation_part == "body":
            spliced_data = havoc_splicing(data, files)
        elif mutation_part == "all":
            spliced_data = havoc_splicing(header + data, files)
        ...
        if len(data) >= InBufferLength:
            ...
            spliced_data = spliced_data[:InBufferLength] # header, all일때는 target.InBuffer_length가 아님
        else:
            spliced_data = spliced_data.ljust(InBufferLength,b"\xff") #TO DO -> make random bytes
            
        if mutation_part == "header":
            target.header = spliced_data
        elif mutation_part == "body":
            target.InBuffer = spliced_data
        elif mutation_part == "all":
            orig_header_len = len(header) # 또는 irp_list[index].header_length
            target.header = spliced_data[:orig_header_len]
            target.InBuffer = spliced_data[orig_header_len:]
        func(irp_list)
