    ...
    def draw(self, cur_rows=None):
        ...
        for i in range(workers_start, workers_end):
            ...
            if d.worker_is_stalled(i):
                stage_info = f"[S] {d.worker_stage(i)[:8]}" # @@ STALLED 뒤에 stage 추가
                self.gui.print_info_line([(14, "", stage_info),
...
class GuiData:
    ...
    def aggregate(self):
        ...
        for nid in self.nodes:
            ...
            if node["info"]["exit_reason"] == "regular":
                ...
                if "fav_bits" in node and len(node["fav_bits"]) > 0: # if len(node["fav_bits"]) > 0:
    ...
    def yield_det(self):
        return (self.stats["yield"].get("afl_arith_1", 0) +
                ...
                self.stats["yield"].get("afl_int_8", 0))
    ...
    def node_payload(self, nid):
        ...
        return read_binary_file(filename)[0:1024]  # TO DO remove path traversal vuln
